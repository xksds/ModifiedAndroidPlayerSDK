...
$CC -lm -lz -shared --sysroot=$FF_SYSROOT -Wl,--no-undefined -Wl,-z,noexecstack $FF_EXTRA_LDFLAGS \
    -Wl,-soname,libNIOFFmpeg.so \
    $FF_C_OBJ_FILES \
    $FF_ASM_OBJ_FILES \
    $FF_DEP_LIBS \
    -o $FF_PREFIX/libNIOFFmpeg.so
...
ln -s $FF_PREFIX/libNIOFFmpeg.so $FF_PREFIX/shared/lib/libNIOFFmpeg.so
...
for f in $FF_PREFIX/lib/pkgconfig/*.pc; do
    ...
    mysedi $f 's/-lavcodec/-lNIOFFmpeg/g'
    mysedi $f 's/-lavfilter/-lNIOFFmpeg/g'
    mysedi $f 's/-lavformat/-lNIOFFmpeg/g'
    mysedi $f 's/-lavutil/-lNIOFFmpeg/g'
    mysedi $f 's/-lswresample/-lNIOFFmpeg/g'
    mysedi $f 's/-lswscale/-lNIOFFmpeg/g'
done
